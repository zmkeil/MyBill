...
public class DBHelper extends SQLiteOpenHelper{
	...
	/* for assets */
	public boolean insert_new_assets(SQLiteDatabase db, String sid, int year, int month, int day, 
	...
	private int update_assets_size() {
		String record_size_file = "assets_record_size";
		int record_size = 0;
		if (file_helper.fileIsExists(record_size_file)) {
			String record_size_line = file_helper.read(record_size_file);
			if (record_size_line != null) {
	...
	private void add_pocket_record(String sid, boolean new_update) {		
		// record size		
		int record_size = update_assets_size();
		    	
		// record
		String record_file = "assets_record";
		String new_record_line = record_size + "POCKET\t" + (new_update ? "\t0\t" : "\t1\t") 
				+ sid + "\n";
		file_helper.append(record_file, new_record_line);
	}
	
	private void add_assets_record(String strid, boolean b) {
		// record size		
		//int record_size = update_assets_size();
		return;
	}
	...
	public Map<String, Object> get_assets_data(SQLiteDatabase db,
			boolean is_pocket, int assets_id) {
		Map<String,Object> map = new HashMap<String, Object>();		
		String sql = "SELECT * FROM " + (is_pocket ? table_name_pocket : table_name_assets) + " where id=?";
		Cursor c = db.rawQuery(sql, new String[]{String.valueOf(assets_id)});
		if(c.moveToFirst()) {
	...
	public boolean update_assets(SQLiteDatabase db, boolean is_pocket,
			int assets_id, String assets_sid, Map<String, Object> update_data) {
		ContentValues cv = new ContentValues();
		if (update_data.containsKey("delete")) {
			cv.put("is_deleted", 1);
		} else {
		...
		if (assets_id >= 0) {
			String whereClause = "id=?";
			String[] whereArgs = {String.valueOf(assets_id)};
			try {
			...
			// update record/index file
			if (assets_sid != null) {
				add_assets_record(assets_sid, false);
			}
		}
		// other update
		else {
			String whereClause = "sid=?";
			String[] whereArgs = {assets_sid};
			try {
				db.update(is_pocket ? table_name_pocket : table_name_assets, cv, whereClause, whereArgs);
			} catch(Exception e) {
	        	Log.i(BWORD, "update other bill failed");
